...
enum h264_rkmpp_encoder_type {
	H264_ENCODER_TYPE_OH264,
};

struct h264_rkmpp_encoder {
	struct ffmpeg_video_encoder ffve;
	enum h264_rkmpp_encoder_type type;

	DARRAY(uint8_t) header;
};

static const char *h264_rkmpp_getname(void *unused)
{
	UNUSED_PARAMETER(unused);
	return "H264_rkmpp";
}

static void h264_rkmpp_video_info(void *data, struct video_scale_info *info)
{
...
static bool h264_rkmpp_update(struct h264_rkmpp_encoder *enc, obs_data_t *settings)
{
	...
	h264_rkmpp_video_info(enc, &info);
...
static void h264_rkmpp_destroy(void *data)
{
	struct h264_rkmpp_encoder *enc = data;
				  ...
				  const char *enc_name)
{
	...
	struct h264_rkmpp_encoder *enc = bzalloc(sizeof(*enc));

	if (strcmp(enc_lib, "h264_rkmpp") == 0)
	...
	if (!ffmpeg_video_encoder_init(&enc->ffve, enc, encoder, enc_lib, NULL, enc_name, NULL, on_first_packet))
		goto fail;
	if (!h264_rkmpp_update(enc, settings))
		goto fail;

	return enc;

fail:
	h264_rkmpp_destroy(enc);
	return NULL;
}

static void *h264_rkmpp_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	return h264_create_internal(settings, encoder, "h264_rkmpp", "H264_rkmpp");
}

static bool h264_rkmpp_encode(void *data, struct encoder_frame *frame, struct encoder_packet *packet,
...
static void h264_rkmpp_defaults(obs_data_t *settings)
{
	obs_data_set_default_int(settings, "bitrate", 2500);
	obs_data_set_default_string(settings, "profile", "main");
}

static obs_properties_t *h264_properties(enum h264_rkmpp_encoder_type type)
{
...
static obs_properties_t *h264_rkmpp_properties(void *unused)
{
	UNUSED_PARAMETER(unused);
	return h264_properties(H264_ENCODER_TYPE_OH264);
}

static bool h264_rkmpp_extra_data(void *data, uint8_t **extra_data, size_t *size)
{
	struct h264_rkmpp_encoder *enc = data;
...
struct obs_encoder_info ffmpeg_h264_rkmpp_encoder = {
	.id = "ffmpeg_h264_rkmpp",
	.type = OBS_ENCODER_VIDEO,
	.codec = "h264",
	.get_name = h264_rkmpp_getname,
	.create = h264_rkmpp_create,
	.destroy = h264_rkmpp_destroy,
	.encode = h264_rkmpp_encode,
	.get_defaults = h264_rkmpp_defaults,
	.get_properties = h264_rkmpp_properties,
	.get_extra_data = h264_rkmpp_extra_data,
	.get_video_info = h264_rkmpp_video_info,
};
