...
enum hevc_rkmpp_encoder_type {
	HEVC_ENCODER_TYPE_OHEVC,
};

struct hevc_rkmpp_encoder {
	struct ffmpeg_video_encoder ffve;
	enum hevc_rkmpp_encoder_type type;

	DARRAY(uint8_t) header;
};

static const char *hevc_rkmpp_getname(void *unused)
{
	UNUSED_PARAMETER(unused);
	return "HEVC_rkmpp";
}

static void hevc_rkmpp_video_info(void *data, struct video_scale_info *info)
{
	UNUSED_PARAMETER(data);

	// OpenHEVC only supports I420
	info->format = VIDEO_FORMAT_I420;
}

static bool hevc_rkmpp_update(struct hevc_rkmpp_encoder *enc, obs_data_t *settings)
{
	...
	hevc_rkmpp_video_info(enc, &info);
...
static void hevc_rkmpp_destroy(void *data)
{
	struct hevc_rkmpp_encoder *enc = data;
...
static void *hevc_create_internal(obs_data_t *settings, obs_encoder_t *encoder, const char *enc_lib,
				  const char *enc_name)
{
	...
	default:; // Make the compiler do the right thing
		const char *const text = obs_module_text("HEVC.UnsupportedVideoFormat");
		obs_encoder_set_last_error(encoder, text);
		blog(LOG_ERROR, "[H.265 encoder] %s", text);
		return NULL;
	}
	...
	case VIDEO_CS_709:
		break;
	default:; // Make the compiler do the right thing
		const char *const text = obs_module_text("HEVC.UnsupportedColorSpace");
		obs_encoder_set_last_error(encoder, text);
		blog(LOG_ERROR, "[H.265 encoder] %s", text);
		return NULL;
	}

	struct hevc_rkmpp_encoder *enc = bzalloc(sizeof(*enc));

	if (strcmp(enc_lib, "hevc_rkmpp") == 0)
		enc->type = HEVC_ENCODER_TYPE_OHEVC;

	if (!ffmpeg_video_encoder_init(&enc->ffve, enc, encoder, enc_lib, NULL, enc_name, NULL, on_first_packet))
		goto fail;
	if (!hevc_rkmpp_update(enc, settings))
		goto fail;

	return enc;

fail:
	hevc_rkmpp_destroy(enc);
	return NULL;
}

static void *hevc_rkmpp_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	return hevc_create_internal(settings, encoder, "hevc_rkmpp", "HEVC_rkmpp");
}

static bool hevc_rkmpp_encode(void *data, struct encoder_frame *frame, struct encoder_packet *packet,
...
static void hevc_rkmpp_defaults(obs_data_t *settings)
{
	obs_data_set_default_int(settings, "bitrate", 2500);
	obs_data_set_default_string(settings, "profile", "main");
}

static obs_properties_t *hevc_properties(enum hevc_rkmpp_encoder_type type)
{
...
static obs_properties_t *hevc_rkmpp_properties(void *unused)
{
	UNUSED_PARAMETER(unused);
	return hevc_properties(HEVC_ENCODER_TYPE_OHEVC);
}

static bool hevc_rkmpp_extra_data(void *data, uint8_t **extra_data, size_t *size)
{
	struct hevc_rkmpp_encoder *enc = data;
...
struct obs_encoder_info ffmpeg_hevc_rkmpp_encoder = {
	.id = "ffmpeg_hevc_rkmpp",
	.type = OBS_ENCODER_VIDEO,
	.codec = "hevc",
	.get_name = hevc_rkmpp_getname,
	.create = hevc_rkmpp_create,
	.destroy = hevc_rkmpp_destroy,
	.encode = hevc_rkmpp_encode,
	.get_defaults = hevc_rkmpp_defaults,
	.get_properties = hevc_rkmpp_properties,
	.get_extra_data = hevc_rkmpp_extra_data,
	.get_video_info = hevc_rkmpp_video_info,
};
